app.controller("studentController", function ($scope, studentService, classesService) {
    ...
    //定义选中数组
    $scope.stuIds = [];
    $scope.showIds=(event,sid)=>{
        if(event.target.checked){
            $scope.stuIds.push(sid);
        }else{
            let index = $scope.selectIds.indexOf(sid);
            $scope.stuIds.splice(index,1);
        }
    }

    //删除所有选中
    $scope.deleteAll = ()=>{
        studentService.deleteAll($scope.stuIds).success(response => {
            if (confirm("确定删除吗？")) {
                if (response.success) {
                    $scope.search();
                } else {
